...
template <int N, int regFileAdrBit, int opcodeBit, int cyclesMDU>
void sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::writeRegTRF(sc_lv<regFileAdrBit> WrADR, sc_lv<N> WrData)
{
...
template <int N, int regFileAdrBit, int opcodeBit, int cyclesMDU>
void sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::readRegTRF(sc_lv<regFileAdrBit> address1, sc_lv<regFileAdrBit> address2)
{
...
template <int N, int regFileAdrBit, int opcodeBit, int cyclesMDU>
void sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::writeRegTRB(sc_lv<regFileAdrBit> WrADR, sc_lv<N> WrData)
{
	sc_uint<regFileAdrBit> wrAd;
	if (TRB_wrReg == '1')
	{
...
template <int N, int regFileAdrBit, int opcodeBit, int cyclesMDU>
void sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::readRegTRB(sc_lv<regFileAdrBit> address1, sc_lv<regFileAdrBit> address2)
{
	sc_uint<regFileAdrBit> ad1, ad2;
	ad1 = address1;
	ad2 = address2;
...
template <int N, int regFileAdrBit, int opcodeBit, int cyclesMDU>
sc_lv<N> sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::nBitSignExtension(sc_lv<N> inputSignal, int leftRange, bool sign)
{
...
template <int N, int regFileAdrBit, int opcodeBit, int cyclesMDU>
sc_lv<N> sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::shiftFunction(sc_lv<5> shiftNumber, sc_lv<N> input, bool logical)
{
	sc_lv<N> result;
	switch (logical)
	{
...
void sayacInstruction<N, regFileAdrBit, opcodeBit, cyclesMDU>::abstractSimulation()
{
	...
	sc_lv<N> Quo, Rem;
	// sc_lv<8> flags;
	sc_lv<N> dist_dx, dist_dy, dist_result;
	sc_lv<N> atan_result;
	sc_uint<N> isqrt_x, isqrt_x1, isqrt_n;
	sc_uint<N> atan_ratio;
	sc_lv<3> RFI;
	sc_logic IR8, IR9;
	PCregister = 0x0000;
	...
	while (true)
	{
		...
		case ODIST:
		{
			cout << "**************DIST Instruction**************" << endl;

			TRF_wrReg = SC_LOGIC_0;
			readRegTRF(rs1ADR, rs2ADR);

			int dx = TRF_RdData1.to_int();
			int dy = TRF_RdData2.to_int();

			unsigned int n = dx * dx + dy * dy;
			...
			{
				unsigned int x = n;
				unsigned int x1 = (x + 1) / 2;

				while (x1 < x)
				{
					x = x1;
					x1 = (x + n / x) / 2;
					for (int i = 0; i < 2 * cyclesMDU + 12; i++)
						wait();
				}
			...
			TRF_wrReg = SC_LOGIC_1;
			writeRegTRF(rdOrrs1, dist_result);
		...
		case OATAN:
		{
			cout << "**************ATAN Instruction**************" << endl;
			TRF_wrReg = SC_LOGIC_0;
			readRegTRF(rs1ADR, rs2ADR);
			dist_dx = TRF_RdData1;
			dist_dy = TRF_RdData2;

			sc_int<N> sdx = dist_dx.to_int();
			sc_int<N> sdy = dist_dy.to_int();
			...
			if (sdx == 0)
			{
				atan_result = (sdy >= 0) ? 90 : -90;
			}
			else
			{
				sc_int<N> ratio = (sdy * 100) / sdx;
				sc_int<N> abs_ratio;
				...
				if (abs_ratio <= 8)
					base_angle = 5;
				else if (abs_ratio <= 17)
					base_angle = 10;
				else if (abs_ratio <= 27)
					base_angle = 15;
				else if (abs_ratio <= 36)
					base_angle = 20;
				else if (abs_ratio <= 47)
					base_angle = 25;
				else if (abs_ratio <= 58)
					base_angle = 30;
				else if (abs_ratio <= 70)
					base_angle = 35;
				else if (abs_ratio <= 84)
					base_angle = 40;
				else if (abs_ratio <= 100)
					base_angle = 45;
				else if (abs_ratio <= 119)
					base_angle = 50;
				else if (abs_ratio <= 143)
					base_angle = 55;
				else if (abs_ratio <= 173)
					base_angle = 60;
				else if (abs_ratio <= 214)
					base_angle = 65;
				else if (abs_ratio <= 275)
					base_angle = 70;
				else if (abs_ratio <= 373)
					base_angle = 75;
				else if (abs_ratio <= 567)
					base_angle = 80;
				else
					base_angle = 85;
				...
				atan_result = base_angle;
			}

			TRF_wrReg = SC_LOGIC_1;
			writeRegTRF(rdOrrs1, atan_result);
			PCregister = PCregister.to_uint() + 1;
			wait();
			break;
		}
